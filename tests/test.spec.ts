...
test.describe("Generating allure reports dashboard", () => {
    test("Generating allure reports dashboard", async ({ page }) => {
        page.goto("link");
        ...
        for (let i = 0; i < list.length; i++) {
            ...
            let timeDifference =
                (currentDate.getTime() - suiteDate.getTime()) /
                (1000 * 60 * 60);

            if (
                chartCaptionValue === "NaN%" ||
                !dateRegex.test(suiteNameValue) ||
                timeDifference >= 2
            ) {
                isOutdatedTest = true;
            }
